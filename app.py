...
def buscar_produtor_por_matricula(matricula: str):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT id, nome, matricula FROM produtores WHERE matricula = %s",
                       (matricula.strip(),))
        row = cursor.fetchone()
        cursor.close()
        conn.close()
...
def api_salvar_entrada():
    ...
    if not produtor_id:
        return jsonify({'sucesso': False, 'mensagem': 'Produtor não selecionado'}), 400
    if not tipo_alho:
        return jsonify({'sucesso': False, 'mensagem': 'Tipo de alho não selecionado'}), 400
    if not detalhes:
        return jsonify({'sucesso': False, 'mensagem': 'Nenhum peso registrado'}), 400

    # Validações por role
    if role == 'classificacao':
        if local_destino != 'Classificação':
            return jsonify({'sucesso': False, 'mensagem': 'Setor Classificação só registra entrada inicial.'}), 400
        local_origem = None
        if horas_banca > 0:
            return jsonify({'sucesso': False, 'mensagem': 'Classificação não permite horas de banca.'}), 400
    elif role == 'banca':
        if local_destino != 'banca':
            return jsonify({'sucesso': False, 'mensagem': 'Setor Banca só transfere para Banca.'}), 400
        if not local_origem or local_origem not in ('Classificação', 'Toletagem'):
            return jsonify({'sucesso': False, 'mensagem': 'Para Banca, a origem deve ser Classificação ou Toletagem.'}), 400
    elif role == 'toletagem':
        if local_destino != 'toletagem':
            return jsonify({'sucesso': False, 'mensagem': 'Setor Toletagem só transfere para Toletagem.'}), 400
        if not local_origem or local_origem not in ('Classificação', 'Banca'):
            return jsonify({'sucesso': False, 'mensagem': 'Para Toletagem, a origem deve ser Classificação ou Banca.'}), 400
